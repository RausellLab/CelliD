...
Cell-ID is based on R version >= 3.6. It contains dependencies with several CRAN and Biocondutor packages as described in the [Description file](https://github.com/cbl-imagine/CellID/blob/master/DESCRIPTION)
...
To illustrate Cell-ID usage we will use throughout this vignette two publickly available pancreas single-cell RNA-seq data sets provided in [Baron et al. 2016](https://doi.org/10.1016/j.cels.2016.08.011) and [Segerstolpe et al. 2016](https://doi.org/10.1016/j.cmet.2016.08.020). 
We provide here for convinience the R objects with the raw counts gene expression matrices and associated metadata
```{r download data}
#read data
...
Library size normalization is carried out by rescaling counts to a common library size of 10000. Log transformation is performed after adding a pseudo-count of 1.  Genes expressing at least one count in less than 5 cells are removed. These steps mimmic the standard Seurat workflow desribed [here](https://satijalab.org/seurat/v3.1/pbmc3k_tutorial.html).
...
At this stage, Cell-ID can perform an automatic cell type prediction for each cell in the dataset. For that purpose, prototypical marker lists associated to well-characterized cell types are used as input, as obtained from third-party sources. Here we will use the Panglao [database](https://panglaodb.se/) of curated gene signatures to predict the cell type of each individual cell in the Baron data. 
